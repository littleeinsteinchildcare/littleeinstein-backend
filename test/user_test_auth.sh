...
create_firebase_users(){
    for (( i = 1; i < ("$NUM_TESTS"+1); i++)) do
        ...
        echo "UID: $uid"
        test_post $idToken "$i"
        test_post_failure_no_auth $idToken "$i"
...
test_post_failure_no_auth(){
	...
	run_test "POST test <No Authorization>" 401
}
...
test_get_failure_no_auth(){
	...
	run_test "GET Test <Unauthorized>" 401
}
